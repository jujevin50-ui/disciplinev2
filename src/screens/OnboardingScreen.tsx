...
export function OnboardingScreen() {
  const { tokens: T, createAccount } = useApp();
  ...
  const finish = () => {
    ...
    createAccount(habits, name.trim() || 'Toi');
    ...
    <div style={{ height: '100%', background: T.paper, color: T.ink, display: 'flex', flexDirection: 'column' }}>
      ...
      {step === 1 && (
        <div style={{ flex: 1, padding: '32px 28px 40px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <h1 style={{ fontSize: 40, fontWeight: 400, lineHeight: 1.05, letterSpacing: -0.8, margin: '0 0 10px' }}>
            Par quoi<br />
            <em style={{ color: T.accent, fontStyle: 'normal' }}>commencer</em> ?
          </h1>
          <p style={{ fontSize: 14, color: T.inkSoft, margin: '0 0 20px' }}>
            Sélectionnez 2 ou 3 habitudes. Vous pouvez modifier à tout moment.
          </p>

          <div className="hide-scrollbar" style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {SUGGESTIONS.map((s, i) => {
              ...
              return (
                <button key={i} onClick={() => toggle(i)} style={{
                  ...
                  borderRadius: 14, color: on ? T.paper : T.ink,
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
            ...
            <button onClick={finish} disabled={selected.size === 0} style={{
              flex: 2, height: 54, borderRadius: 14,
              background: selected.size > 0 ? T.accent : T.paperAlt,
              color: selected.size > 0 ? '#fff' : T.inkMuted,
              fontSize: 15, fontWeight: 600,
            }}>
              C'est parti →
            </button>
